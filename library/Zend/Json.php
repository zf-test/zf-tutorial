...
class Zend_Json
{
    ...
    public static function encode($valueToEncode, $cycleCheck = false, $options = array())
    {
        ...
        if (function_exists('json_encode') && self::$useBuiltinEncoderDecoder !== true) {
            try{
                ...
                $foo=error_get_last(); //json_last_error() does not work correctly on arrays
                if(isset($foo['message']) && $foo['message']=='json_encode(): Invalid UTF-8 sequence in argument'){
                    // eventually remove this - just sampling because there are so many errors
                    if (!rand(0, 9999)){ // one out of every 10K times
                        $truncatedValue = is_string($valueToEncode) ? $valueToEncode : serialize($valueToEncode);
                        $truncatedValue = substr($truncatedValue, 0, 100);
                        $page = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'no referer';
                        error_log("${foo['message']}\n[$truncatedValue]\n$page");
                    }
                    if (is_array($valueToEncode)) {
                        array_walk_recursive($valueToEncode, array(__CLASS__,"array_utf8_encode"));
                    } else {
                        self::string_utf8_encode($valueToEncode);
                    }
                    $encodedResult = json_encode($valueToEncode);
                    @trigger_error('');// this clears the error message
                }
            }catch(Exception $e){
                if($e->getMessage()=='json_encode(): Invalid UTF-8 sequence in argument'){
                    if (is_array($valueToEncode)) {
                        array_walk_recursive($valueToEncode, array(__CLASS__,"array_utf8_encode"));
                    } else {
                        self::string_utf8_encode($valueToEncode);
    ...
    public static function array_utf8_encode(&$val, $key){
        if(is_string($val)){
            self::string_utf8_encode($val);
        }
    }

    public static function string_utf8_encode(&$val){
        $s1 = mb_convert_encoding($val, "UTF-8", "UTF-8" );
        if(strlen($s1)!=strlen($val)){
            $val = $s1;
        }
    }
}
