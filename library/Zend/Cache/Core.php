...
class Zend_Cache_Core
{
     ...
     */
    public function remove($id, $log_error = false)
    {
        if (!$this->_options['caching']) {
            return true;
        }
        $id = $this->_id($id); // cache id may need prefix
        self::_validateIdOrTag($id);
        return $this->_backend->remove($id, $log_error);
    }
    ...
    protected static function _validateIdOrTag($string)
    {
        if (!is_string($string) || '' == trim($string)) {
            error_log('Invalid id or tag : must be a string');
            Zend_Cache::throwException('Invalid id or tag : must be a string');
        }

        if (substr($string, 0, 9) == 'internal-') {
            error_log('"internal-*" ids or tags are reserved');
            Zend_Cache::throwException('"internal-*" ids or tags are reserved');
        }
//        if (!preg_match('~^[\w]+$~D', $string)) {
//            error_log("Invalid id or tag '$string' : must use only [a-zA-Z0-9_]"));
