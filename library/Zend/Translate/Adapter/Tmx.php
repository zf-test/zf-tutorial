...
class Zend_Translate_Adapter_Tmx extends Zend_Translate_Adapter {
    ...
    protected function _loadTranslationData($filename, $locale, array $options = array())
    {
        ...
        //get a list of ALL locales in this this tmx file
        $locales=array_keys($this->_data);unset($locales['en']);

        // Fallback to english if no translation is found for each tuid in all the other locales
        foreach($this->_data['en'] as $tuid=>$phrase){
            foreach($locales as $locale2){
                if(!isset($this->_data[$locale2][$tuid]) || empty($this->_data[$locale2][$tuid])){
                    $this->_data[$locale2][$tuid]=$phrase;
                }
            }
        }
