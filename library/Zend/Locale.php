...
class Zend_Locale
{
    ...
    private static $_localeData = array(
        ...
        'bn'    => true, 'bo_CN' => true, 'bo_IN' => true, 'bo'    => true, 'bp'=>TRUE,'bs_BA' => true,
        ...
        'de'    => true, 'dt'=>true,'dv_MV' => true, 'dv'    => true, 'dz_BT' => true, 'dz'    => true,
    ...
    public static function getInstance($locale = null){
        if($locale===null){
            if (isset(self::$instance['null'])) {
                ...
                self::$instance['null'] = new Zend_Locale();
                return self::$instance['null'];
            }
        }else{
            if (!isset(self::$instance[$locale])) {
            	self::$instance[$locale] = new Zend_Locale($locale);
            }
        }
    ...
    public static function isLocale($locale, $strict = false, $compatible = true)
    {
        ...
        if(isset(self::$cache_isLocale[$key][$strict][$compatible])){
            return self::$cache_isLocale[$key][$strict][$compatible];
        }

        try {
            $locale = self::_prepareLocale($locale, $strict);
        } catch (Zend_Locale_Exception $e) {
            self::$cache_isLocale[$key][$strict][$compatible]=false;
            return false;
        }
        ...
        if (($compatible === true) and (self::$compatibilityMode === true)) {
            ...
            if (isset(self::$_localeData[$locale]) === true) {
                self::$cache_isLocale[$key][$strict][$compatible]=$locale;
                return $locale;
            } else if (!$strict) {
                $locale = explode('_', $locale);
                if (isset(self::$_localeData[$locale[0]]) === true) {
                    self::$cache_isLocale[$key][$strict][$compatible]=$locale[0];
                    return $locale[0];
                }
            }
        } else {
            if (isset(self::$_localeData[$locale]) === true) {
                self::$cache_isLocale[$key][$strict][$compatible]=true;
                return true;
            } else if (!$strict) {
                $locale = explode('_', $locale);
                if (isset(self::$_localeData[$locale[0]]) === true) {
    ...
    public static function findLocale($locale = null)
    {
        ...
        if ($locale === null) {
            ...
            $locale=Zend_Locale::getInstance();
        }
        ...
        if (!Zend_Locale::isLocale($locale, true, false)) {
            ...
            $locale=Zend_Locale::getInstance($locale);
        }

        $locale = self::_prepareLocale($locale);

        self::$locales[$key]=$locale;
