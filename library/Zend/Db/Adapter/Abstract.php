...
abstract class Zend_Db_Adapter_Abstract
{
     ...
     * @return array
     */
    public function fetchAll($sql, $bind = array(), $fetchMode = null,$className='stdClass')
    {
        if ($fetchMode === null) {
        ...
        //$st=microtime(TRUE);
        $stmt = $this->query($sql, $bind);
        //$rt=microtime(TRUE)-$st; // get the 'response time'. The amount of time it took to execute this query
        //$this->saveResponseTime($rt);
        $result = $stmt->fetchAll($fetchMode,null,$className);
        return $result;
    }
     ...
     * @param mixed $bind Data to bind into SELECT placeholders.
     * @param mixed                 $fetchMode Override current fetch mode.
     * @param string $className - the name of the class to create when using fetchMode FETCH_CLASS
     * @return array
     */
    public function fetchRow($sql, $bind = array(), $fetchMode = null,$className='stdClass')
    {
        ...
        //$rt=microtime(TRUE)-$st; // get the 'response time'. The amount of time it took to execute this query
        //$this->saveResponseTime($rt);
        $result = $stmt->fetch($fetchMode,null,null,$className);
        return $result;
    }
    ...
    /**
     * Saves the response time of a sql query into local memory for use by the load balancer
     * @see ZendmodoDbTableAbstract::getReadonlyAdapter
     * @param float $rt - The amount of time it took to execute a sql query.
     */
     /*public function saveResponseTime($rt){
        if(function_exists('xcache_get')){
            $lb_stats=xcache_get('LB_STATS');
            if(!is_array($lb_stats)){
                $lb_stats=array();
            }
            if(isset($lb_stats[$this->_config['name']])){
                    //calculate a smoothed average response time
                    $lb_stats[$this->_config['name']]=array(( 0.8 * $lb_stats[$this->_config['name']][0] ) + ( 1 - 0.8 ) * $rt,time());
            }else{
                    $lb_stats[$this->_config['name']]=array($rt,time());
            }
            xcache_set('LB_STATS',$lb_stats);
        }
    }*/
}
